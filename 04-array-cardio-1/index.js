...
const sortedByBirth = inventors.slice().sort((a, b) => {
...
const amountOfYearsLived = inventors.reduce((count, inventor) => {
  return count + inventor.passed - inventor.year;
}, 0);

console.log(amountOfYearsLived);
...
const instances = data.reduce((result, item) => {
  result[item] = typeof result[item] === 'number' ? result[item] + 1 : 1;
